...
public class ContextMenu extends PopupMenu implements ActionListener {
    ...
    private final static String CLONE_BLOCK_THIS = "CLONEthis";	//heqichen
    private final static String CLONE_BLOCK_RIGHT = "CLONEright";	//heqichen
    private final static String CLONE_BLOCK_ALL = "CLONEall";	//heqichen
    private static MenuItem cloneItem = null;	//heqichen
    ...
    private static void initAddCommentMenu() {
    	...
    	/*cloneItem = new MenuItem(uiMessageBundle.getString("ardublock.ui.clone"));
    	cloneItem.setActionCommand(CLONE_BLOCK);
    	cloneItem.addActionListener(rndBlockMenu);
        addCommentMenu.add(cloneItem);*/

        cloneItem1 = new MenuItem(uiMessageBundle.getString("ardublock.ui.clone_this"));
    	cloneItem1.setActionCommand(CLONE_BLOCK_THIS);
    	cloneItem1.addActionListener(rndBlockMenu);
        addCommentMenu.add(cloneItem1);
        
    	cloneItem2 = new MenuItem(uiMessageBundle.getString("ardublock.ui.clone_right"));
    	cloneItem2.setActionCommand(CLONE_BLOCK_RIGHT);
    	cloneItem2.addActionListener(rndBlockMenu);
        addCommentMenu.add(cloneItem2);
        
    	cloneItem3 = new MenuItem(uiMessageBundle.getString("ardublock.ui.clone_all"));
    	cloneItem3.setActionCommand(CLONE_BLOCK_ALL);
    	cloneItem3.addActionListener(rndBlockMenu);
        addCommentMenu.add(cloneItem3);
    ...
    public void actionPerformed(ActionEvent a) {
        ...
        } else if (a.getActionCommand() == CLONE_BLOCK_THIS) {
        	//notify the renderableblock componenet that lauched the conetxt menu
            if (activeComponent != null && activeComponent instanceof RenderableBlock) {
                //((RenderableBlock) activeComponent).cloneMe();
            	((RenderableBlock) activeComponent).cloneMe(activeComponent);
            }
        } else if (a.getActionCommand() == CLONE_BLOCK_RIGHT) {
        	//notify the renderableblock componenet that lauched the conetxt menu
            if (activeComponent != null && activeComponent instanceof RenderableBlock) {
                //((RenderableBlock) activeComponent).cloneMe();
            	((RenderableBlock) activeComponent).cloneMe(activeComponent, true);
            }
        } else if (a.getActionCommand() == CLONE_BLOCK_ALL) {
        	//notify the renderableblock componenet that lauched the conetxt menu
            if (activeComponent != null && activeComponent instanceof RenderableBlock) {
                //((RenderableBlock) activeComponent).cloneMe();
            	((RenderableBlock) activeComponent).cloneMe(activeComponent, true, true);
            }
        }
    }
}
